...
const AllTripCard = ({ data }) => {
  const { categoryName, image, location, rating, duration, title, id } = data;
  return (
    <div className="">
      <Helmet>
        <title>All-Trips-pages!</title>
      </Helmet>
      <div className="max-w-[800px] relative">
        <img
          className="w-[400px] h-[300px] md:w-[700px] md:h-[400px] mx-auto rounded-lg"
          src={image}
          alt={categoryName}
        />
        <div className="absolute bottom-2 left-5 md:bottom-6 md:left-5 space-y-1">
          ...
          <Link to={`/detail/${id}`}>
            <button className="py-1 px-3 bg-orange-300 rounded-lg hover:bg-gray-200 cursor-pointer">
              Explore Trips
            </button>
          </Link>
        </div>
